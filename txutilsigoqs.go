...
import (
	...
	//"github.com/hyperledger/fabric/protos/sigoliboqs"
	"github.com/hyperledger/fabric/txutilsigoqs"
)
...
func GetSignedProposal(prop *peer.Proposal, signer msp.SigningIdentity) (*peer.SignedProposal, error) {
	...
	lib, _ := txutilsigoqs.LoadLib("/usr/local/lib/liboqs.so")
	defer lib.Close()

	//Set the signature algorithm to be used .                             (Use q-tesla)

	sig, err := lib.GetSign(txutilsigoqs.SigqTESLAI)
	if err != nil {
		return nil, err
	}

	//To generate one PQ- key-pair...

	publicKey, secretKey, err2 := sig.KeyPair()
	defer sig.Close()
	...
	//The following is a generated Classical signature.
	//CLsignature, err := signer.Sign(propBytes)...
	signature, err := signer.Sign(propBytes)
	if err != nil {
		return nil, err
	}
	fmt.Println(signature)
	//fmt.Println("dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd")
	//To sign the proposal using PQ-key...
	//**********************************************	PQsignature, err := testSIG.Sign(secretKey, propBytes)

	//Concatenate both PQ and Classical signatures...
	// NOTE: The former bytes are classical and latter ones are PQ...

	PQsignature, err3 := sig.Sign(secretKey, propBytes)
	if err3 != nil {
		return nil, err3
	}
	fmt.Println(PQsignature)

	signature = append(signature, PQsignature...)

	fmt.Println(signature)
	//Another to-do : Print PQ signature here and also in the verification side to see if it matches...
